const CACHE_NAME = 'nozio-cache-v3';
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) => {
      ...
      // clients.claim() MUSS innerhalb von waitUntil() stehen,
      // damit es auf iOS Safari zuverlässig greift
  ...
  event.respondWith(
    caches.match(event.request).then((cachedResponse) => {
      ...
      return fetch(event.request).then((networkResponse) => {
        ...
        // Dynamischer Pfad: funktioniert auch bei GitHub Pages Sub-Path /Nozio/
        if (event.request.mode === 'navigate') {
          const indexUrl = self.registration.scope + 'index.html';
          return caches.match(indexUrl).then((r) => r || caches.match('./index.html'));
        }
      });
    })
  );
